...
export class AnalyticsController {
  ...
  stats = async (req: AuthRequest, res: Response, next: NextFunction) => {
    try { const data = await this.service.stats(req.user?.userId as string, req.query as any); res.json(ApiSuccess.item(data)); } catch (err) { next(err); }
  };
  ...
  listEvents = async (req: AuthRequest, res: Response, next: NextFunction) => {
    try { const items = await this.service.listEvents(req.user?.userId as string, req.params.id, req.query?.type as string | undefined); res.json(ApiSuccess.item({ items })); } catch (err) { next(err); }
  };

  addEvent = async (req: AuthRequest, res: Response, next: NextFunction) => {
    try { const data = await this.service.addEvent(req.user?.userId as string, req.params.id, req.body, req.user?.userId); res.status(201).json(ApiSuccess.created(data)); } catch (err) { next(err); }
  };
}
