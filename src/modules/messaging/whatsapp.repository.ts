...
export class WhatsappRepository {
  ...
  async getConfig(tenantId: string) {
    const row = await this.Config.findOne({ where: { tenant_id: tenantId } });
    return row ? row.toJSON() : null;
  }
  async upsertConfig(tenantId: string, dto: WhatsappConfigDTO) {
    const existing = await this.Config.findOne({ where: { tenant_id: tenantId } });
    if (!existing) {
      const created = await this.Config.create({ tenant_id: tenantId, instance_id: dto.instanceId, instance_name: dto.instanceName, phone_number: dto.phoneNumber, is_active: dto.isActive ?? true, webhook_url: dto.webhookUrl });
  ...
  async listContacts(tenantId: string, search?: string, page = 1, pageSize = 20) {
  ...
  async getContact(tenantId: string, id: string) {
    const row = await this.Contact.findOne({ where: { id, tenant_id: tenantId } });
    return row ? row.toJSON() : null;
  }
  async createContact(tenantId: string, dto: WhatsappContactDTO) {
    const created = await this.Contact.create({ tenant_id: tenantId, phone_number: dto.phoneNumber, name: dto.name });
    return { id: created.id, phoneNumber: created.phone_number, name: created.name };
  }

  async listMessages(tenantId: string, contactId: string, before?: string, after?: string, limit = 50) {
    const where: any = { tenant_id: tenantId, contact_id: contactId };
  ...
  async sendMessage(tenantId: string, contactId: string, dto: WhatsappMessageDTO, fromMe = true) {
    const created = await this.Message.create({ tenant_id: tenantId, contact_id: contactId, sender: fromMe ? 'me' : 'contact', message_type: dto.messageType, content: dto.content, media_url: dto.mediaUrl, file_name: dto.fileName, from_me: fromMe, status: 'sent', message_timestamp: new Date(), raw_payload: {} });
    return created.toJSON();
  }

  async stats(tenantId: string) {
    const [totalMessages, totalContacts, todayMessages] = await Promise.all([
      this.Message.count({ where: { tenant_id: tenantId } }),
      this.Contact.count({ where: { tenant_id: tenantId } }),
      this.Message.count({ where: { tenant_id: tenantId, message_timestamp: { [Op.gte]: new Date(new Date().toDateString()) } } }),
    ]);
