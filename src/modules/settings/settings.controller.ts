...
export class SettingsController {
  ...
  getBusiness = async (req: AuthRequest, res: Response, next: NextFunction) => {
    try { const data = await this.service.getBusiness(req.user?.userId as string); res.json(ApiSuccess.item(data)); } catch (err) { next(err); }
  };
  upsertBusiness = async (req: AuthRequest, res: Response, next: NextFunction) => {
    try { const data = await this.service.upsertBusiness(req.user?.userId as string, req.body); res.json(ApiSuccess.updated(data)); } catch (err) { next(err); }
  };

  getNotification = async (req: AuthRequest, res: Response, next: NextFunction) => {
    try { const data = await this.service.getNotification(req.user?.userId as string); res.json(ApiSuccess.item(data)); } catch (err) { next(err); }
  };
  upsertNotification = async (req: AuthRequest, res: Response, next: NextFunction) => {
    try { const data = await this.service.upsertNotification(req.user?.userId as string, req.body); res.json(ApiSuccess.updated(data)); } catch (err) { next(err); }
  };
