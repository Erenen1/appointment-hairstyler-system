...
export const updateSiteSettingsSchema = Joi.object({
  ...
  appointmentSettings: Joi.object({
    minAdvanceBooking: Joi.number().integer().min(1).max(7).optional(), 
    maxAdvanceBooking: Joi.number().integer().min(7).max(365).optional(), 
    slotDuration: Joi.number().integer().valid(15, 30, 45, 60).optional(), 
    bufferTime: Joi.number().integer().min(0).max(60).optional(), 
    allowCancellation: Joi.boolean().optional(),
    cancellationDeadline: Joi.number().integer().min(1).max(48).optional(), 
    requireConfirmation: Joi.boolean().optional(),
    sendReminders: Joi.boolean().optional(),
    reminderTime: Joi.number().integer().valid(24, 12, 2, 1).optional() 
  }).optional(),
